...
class WikiDataItem():
    ...
    * different ways to write it
    ...
    def __init__(self,response):
        ...
        #if the entry not found, then we return None
    ...
    def _collect_connections(self,data):
        ...
        for i in range(len(data)):
            if self._check_structure(data[i]):
    ...
    def _check_structure(self,el):
        """Function checks if an element contains data value that has an id"""
        if "mainsnak" not in el:
            return False
        
        if "datavalue" not in el["mainsnak"]:
            return False

        if "value" not in el["mainsnak"]["datavalue"]:
            return False
        
        if "Id" not in [str(x).capitalize() for x in el["mainsnak"]["datavalue"]["value"]]:
            return False
