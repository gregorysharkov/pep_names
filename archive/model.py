...
class SiameseBiLSTM:
    ...
    def train_model(self, names_pair, is_similar, embedding_meta_data, model_save_directory='./'):
        """
        Train Siamese network to find similarity between names in `names_pair`
            Steps Involved:
                1. Pass the each from names_pairs  to bidirectional LSTM encoder.
        ...
        Args:
            names_pair (list): list of tuple of name pairs
            is_similar (list): target value 1 if same names pair are similar otherwise 0
        ...
        val_data_x1, val_data_x2, val_labels, leaks_val = create_train_dev_set(tokenizer, names_pair,
        ...
        # Creating LSTM Encoder layer for First name
        sequence_1_input = Input(shape=(self.max_sequence_length,), dtype='int32')
        embedded_sequences_1 = embedding_layer(sequence_1_input)
        x1 = lstm_layer(embedded_sequences_1)

        # Creating LSTM Encoder layer for Second name
        ...
        # Merging two LSTM encodes vectors from names to
    ...
    def update_model(self, saved_model_path, new_names_pair, is_similar, embedding_meta_data):
        """
        Update trained siamese model for given new names pairs 
            Steps Involved:
                1. Pass the each from names from new_names_pair to bidirectional LSTM encoder.
                2. Merge the vectors from LSTM encodes and passed to dense layer.
                3. Pass the  dense layer vectors to sigmoid output layer.
                4. Use cross entropy loss to train weights
        Args:
            model_path (str): model path of already trained siamese model
            new_names_pair (list): list of tuple of new names pairs
            is_similar (list): target value 1 if same names pair are similar otherwise 0
            embedding_meta_data (dict): dict containing tokenizer and word embedding matrix
        Returns:
            return (best_model_path):  path of best model
        ...
        train_data_x1, train_data_x2, train_labels, leaks_train, \
        val_data_x1, val_data_x2, val_labels, leaks_val = create_train_dev_set(tokenizer, new_names_pair,
                                                                               is_similar, self.max_sequence_length,
                                                                               self.validation_split_ratio)
